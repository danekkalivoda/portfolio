        ...
        <nav class="flex flex-wrap justify-between sm:justify-start items-center bg-gray-200 dark:bg-gray-900 transition-bg py-4 px-8 sm:p-8">
          <div class="flex flex-row items-center text-left w-auto">
            ...
            <div class="fixed left-0 top-0 z-40 ml-16 sm:ml-0 sm:rotate-90 transform-bl">
              <span class="inline-block sm:rotate-180 my-6 sm:my-12 pr-2 pl-2 sm:pl-8  bg-gray-200 dark:bg-gray-900 text-sm sm:text-lg uppercase font-bold tracking-tight text-green-500 transition-bg">
          ...
          <ul
            ...
            >
              <li class="sm:inline-flex" v-for="item in $static.navigation.result" :key="item.order">
                <a v-if="$route.path === '/'" :href="'/' + item.url" v-scroll-to="item.url" class="text-right sm:text-left text-gray-700 dark:text-gray-400 hover:text-green-500 flex-1 text-center py-3 sm:py-1 px-3 block">{{ item.title }}</a>
