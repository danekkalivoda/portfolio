<template >
<div class="sm:pl-32 sm:pr-16 bg-gray-200 dark:bg-gray-900 w-full transition-bg duration-150 ease-in">
  <div class="content-wrapper leading-normal flex flex-col min-h-screen ">
    <header class="w-full sm:-ml-32 fixed z-60">
      <nav class="flex flex-wrap justify-between sm:justify-start items-center bg-gray-200 dark:bg-gray-900 transition-bg duration-150 ease-in py-4 px-8 sm:p-8 relative">
        <div class="flex flex-row items-center text-left w-auto">
          ...
          <div class="fixed left-0 top-0 z-40 ml-16 sm:ml-0 transform sm:rotate-90 origin-bottom-left z-10">
            <span class="inline-block sm:rotate-180 transform my-6 sm:my-12 pr-2 pl-2 sm:pl-8 bg-gray-200 dark:bg-gray-900 text-sm sm:text-lg uppercase font-semibold tracking-tight text-green-500 transition-bg duration-150 ease-in">
                <span class="transform -translate-y-full">
                  {{$static.site.SiteSetting.title}}
                </span>
              </span>
          </div>
        </div>
        ...
        <div class="fixed left-0 right-0 top-0 min-w-screen h-16 sm:h-32 z-10 pt-1 bg-gray-200 dark:bg-gray-900 sm:pt-0 transition-all duration-150 ease-in">
          <div class="h-full self-stretch mx-16 sm:ml-32 sm:mr-16 border-b border-gray-400 dark:border-gray-800 transition-all duration-150 ease-in transform -translate-y-1/2"></div>
        </div>
        <ul
            class="uppercase tracking-tight w-full sm:w-auto text-sm flex-col sm:flex-row sm:flex w-auto sm:justify-center bg-gray-200 dark:bg-gray-900 sm:px-5 transition-all duration-150 ease-in z-0 block absolute left-0 top-full sm:relative sm:z-20 transform"
            :class="isOpen ? 'translate-y-0 sm:translate-y-0': '-translate-y-full sm:translate-y-0'"
          >
            <li class="sm:inline-flex" v-for="item in $static.navigation.findNavigation.result" :key="item.order">
    ...
    <footer class="bg-gray-200 dark:bg-gray-900 transition-bg duration-150 ease-in h-24 z-40">
      <div class="c-container mx-auto flex flex-col lg:flex-row items-center justify-between pt-8">
        <div class="text-gray-700 dark:text-gray-500 text-center text-sm md:text-base px-16 w-full editor" v-html="$static.site.SiteSetting.footer_text"></div>
      </div>
    </footer>
  </div>
</div>
